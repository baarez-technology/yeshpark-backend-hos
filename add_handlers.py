"""Script to add new handler methods to admin_ai_assistant.py"""
import re
...
if '_handle_assign_room' in content:
    print('Handlers already exist!')
    exit(0)

# Find the insertion point - after _handle_assign_task ends
# We look for the pattern where _handle_send_email starts
search_pattern = r'(                error=result\.error\n            \)\n\n    async def _handle_send_email\()'

# New handler methods to insert
new_handlers = '''                error=result.error
            )
    ...
    async def _handle_assign_room(
        ...
        if not room_id:
            return AdminAIResponse(
                ...
                confidence=confidence,
                session_id=session_id,
                suggestions=["Show available rooms", "Show rooms status"]
            )
        ...
        if room_status not in ["available", "clean", "inspected"]:
            return AdminAIResponse(
                message=f"Room {room_number} is currently **{room_status}**. Only available/clean rooms can be assigned.",
                intent=intent,
                confidence=confidence,
                session_id=session_id,
        ...
        booking_query = await self.sql_executor.execute_raw(
            """SELECT b.id, b.booking_number, b.confirmation_code, b.status, b.room_id,
    ...
    async def _handle_transfer_room(
        ...
        if not to_room:
            return AdminAIResponse(
                message="Please specify the target room. Example: 'Transfer guest from room 501 to room 502'",
                intent=intent,
                confidence=confidence,
                session_id=session_id,
                suggestions=["Show available rooms"]
        ...
        if not target_room_query.success or not target_room_query.data:
            return AdminAIResponse(
                message=f"Target room {to_room} not found.",
                intent=intent,
                confidence=confidence,
                session_id=session_id,
        ...
        if from_room:
            source_room_query = await self.sql_executor.execute_raw(
                "SELECT id, number FROM rooms WHERE number = :room_num OR id = :room_id LIMIT 1",
                {"room_num": str(from_room), "room_id": from_room}
            )
            if source_room_query.success and source_room_query.data:
                source_room_id = source_room_query.data[0].get("id")
                source_room_number = source_room_query.data[0].get("number")

                booking_query = await self.sql_executor.execute_raw(
                    """SELECT b.id, b.booking_number, b.room_id, g.first_name, g.last_name
                       FROM bookings b
                       LEFT JOIN guests g ON b.guest_id = g.id
                       WHERE b.room_id = :room_id AND b.status = 'checked_in'
                       LIMIT 1""",
        ...
        action = ActionRequest(
            ...
            params={
                "booking_id": booking_db_id,
                "from_room_id": booking.get("room_id"),
                "to_room_id": target_room_id,
                "from_room_number": source_room_number,
        ...
        return AdminAIResponse(
            message=f"Transfer guest:\\n\\n"
                    f"- **Guest:** {guest_name}\\n"
                    f"- **From Room:** {source_room_number}\\n"
                    f"- **To Room:** {target_room_number}\\n\\n"
                    f"Confirm?",
            intent=intent,
            confidence=confidence,
            session_id=session_id,
    ...
    async def _handle_create_guest_note(
        ...
        return AdminAIResponse(
            message=f"Add note to guest:\\n\\n"
                    ...
                    f"- **Note:** {note_text[:100]}{'...' if len(note_text) > 100 else ''}\\n\\n"
                    f"Confirm?",
            intent=intent,
            confidence=confidence,
            session_id=session_id,
...
new_content = re.sub(search_pattern, new_handlers, content)

if new_content == content:
    print('ERROR: Pattern not found! Please check file structure.')
    exit(1)
